<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<%@ taglib prefix="s" uri="http://www.springframework.org/tags"%>
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt"%>
<%@page contentType="text/html" pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
    <head>
        <title>Index Page</title>
        <!-- Bootstrap core CSS -->
        <link href="${pageContext.request.contextPath}/css/bootstrap.min.css" rel="stylesheet">

        <!-- Custom styles for this template -->
        <!--<link href="${pageContext.request.contextPath}/css/starter-template.css" rel="stylesheet">-->

        <!-- SWC Icon -->
        <link rel="shortcut icon" href="${pageContext.request.contextPath}/img/icon.png">

    </head>
    <body>
        <div class="container">

            <div class="row">
                <div class="text-center">
                    <h1>File Successfully Uploaded</h1>
                </div>
            </div>            
            <div class="row">
                <div class="col-sm-6 text-right">File Name: </div>
                <div class="col-sm-6 text-left">${originalName}</div>
            </div>
            <div class="row">
                <div class="col-sm-6 text-right">File Size: </div>
                <div class="col-sm-6 text-left">${fileSize}</div>
            </div>
            <div class="row">
                <div class="col-sm-6 text-right">File Type: </div>
                <div class="col-sm-6 text-left">${contentType}</div>
            </div>

            <div class="row">
                <div class="text-center">
                    <h1><a href="${pageContext.request.contextPath}/image/imageadmin">Click Here To Return To The Image Admin Page</a></h1>
                </div>
            </div>        



        </div>
        <!-- Placed at the end of the document so the pages load faster -->
        <script src="${pageContext.request.contextPath}/js/jquery-1.11.1.min.js"></script>
        <script src="${pageContext.request.contextPath}/js/bootstrap.min.js"></script>

    </body>
</html>

